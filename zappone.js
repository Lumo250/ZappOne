...
// FIX: sgancia SOLO le immagini di un container specifico dall'observer
// condiviso, invece di disconnettere l'intero observer (usato anche da
// altre liste, es. canali vs guida EPG). Va richiamata prima di svuotare un
// container con immagini lazy-load, per rilasciare i riferimenti ai nodi
// DOM in uscita senza "orfanare" le immagini non ancora caricate di altri
// container ancora vivi.
...
// ================================================================
// SWITCH DI VISIBILITÀ TRA VISTE PRINCIPALI
// ================================================================
// Punto unico che decide quali container nascondere/mostrare per ogni
// vista. Prima questa lista era duplicata (es. il listener di fullEpgBtn
// e l'inizio di renderFullEPGList facevano la stessa cosa due volte).
// Le due voci sotto rispecchiano ESATTAMENTE cosa nascondeva/mostrava
// ciascuna destinazione prima di questo refactoring, inclusa la
// differenza che epgManager non tocca #playerContainer mentre fullEpg sì.
...
// ================================================================
// RENDERER GENERICO PER LISTE "RAGGRUPPATE E SALVATE"
// ================================================================
// Disegna gruppi con righe in un container senza sapere se sta disegnando
// EPG, playlist o altro: ogni comportamento specifico arriva da `config`.
// Oggi la usa solo l'EPG manager; è scritta per poter accogliere in futuro
// anche le playlist senza essere riscritta. Non tocca né chiama
// renderGroupedChannelList: canali, preferiti e playlist restano invariati.
//
// config = {
//   emptyCheck: (groups) => bool,
//   emptyState: { image, alt, onClick },
//   onItemClick: (itemData, itemEl) => void,
//   itemActions: (itemData) => [ { icon, title, className, onClick(itemData, itemEl, iconEl) } ],
//   buildInfoLine: (itemData) => string | null,
//   onComplete: () => void   // chiamata quando il disegno (anche a blocchi) è finito davvero
// }
function renderSavedItemsList(containerEl, groups, config = {}) {
  ...
  requestAnimationFrame(() => {
    ...
    const createItem = (itemData, groupIndex) => {
      ...
      if (config.itemActions) {
        config.itemActions(itemData).forEach(action => {
          ...
          // Se la lista viene ridisegnata MENTRE si è già in modalità
          // cancellazione (es. dopo aver eliminato un elemento), la nuova
          // icona nasce già visibile: non serve un nuovo long-press.
    ...
    // Rendering a blocchi (stesso pattern di renderGroupedChannelList): non
    // blocca il thread principale se la lista dovesse diventare grande.
            ...
            // --- GESTORE CLICK PLAYLIST SALVATE ---
                ...
                item.onclick = async () => {
                    try {
                        ...
                        // Evidenzia SUBITO la playlist cliccata, prima ancora
                        // di scaricare o attivare il contenuto (stessa logica
                        // già applicata all'EPG manager).
...
function renderGroupedChannelList(groups, options = {}) {
    ...
    // NUOVO: la modalità griglia è consentita SOLO alla lista canali.
    // Playlist, EPG manager e Full EPG sono sempre renderizzati come lista.
    ...
    // FIX: prima si disconnetteva l'INTERO observer condiviso qui, il che
    // orfanava anche le immagini non ancora caricate della guida EPG (che usa
    // lo stesso observer). Ora si sganciano solo le immagini di QUESTO
    // container, lasciando intatte quelle delle altre liste.
    unobserveContainerImages(targetElement);

    requestAnimationFrame(() => {
    const isGridView = effectiveViewMode === 'grid';        // era currentViewMode
    const viewModeClass = 'view-' + effectiveViewMode;      // era currentViewMode
...
item.className = `channel-item ${effectiveViewMode}`;   // era currentViewMode
...
const groupTitle = document.createElement('div');
            ...
            const groupContent = document.createElement('div');
 groupContent.className = `group-content ${effectiveViewMode}-view`; // era currentViewMode
...
async function showChannelList() {
    ...
    // FIX: chiude anche il drawer dei programmi di un singolo canale EPG,
    // se rimasto aperto. Si chiude altrimenti solo tramite la propria X
    // interna; tornando ai canali da un'altra via (es. la bottombar) restava
    // aperto e, avendo z-index 3000 e sfondo opaco, copriva il player al
    // successivo avvio di un canale — video nascosto, audio regolarmente udibile.
    ...
    // FIX: rimuove la classe 'hidden' (display:none !important) che il
    // pulsante "EPG Completo" applica a #playerContainer all'apertura.
    // Nulla la rimuoveva tornando ai canali per altre vie (drawer, bottombar):
    // restava per il resto della sessione e, per via di !important, vinceva
    // sempre sullo style inline che il player usa per rendersi visibile —
    // da qui video invisibile ma audio regolare ad ogni riproduzione successiva.
...
function processSwipe(dx, dy) {
  ...
  if (document.getElementById('channelEpgDrawer')?.classList.contains('open')) return;
    ...
    // NUOVO: lo swipe cambia list/grid SOLO nella vista canali.
    // Playlist, EPG manager e Full EPG sono sempre list-view → ignora lo swipe.
    const channelListContainer = document.getElementById('channelListContainer');
...
function switchViewMode(mode) {
    // 1. Aggiorna la preferenza globale (vale SOLO per la lista canali).
    //    Playlist, EPG manager e Full EPG ignorano currentViewMode e restano
    //    sempre in list-view: la preferenza viene applicata esclusivamente da
    //    renderGroupedChannelList quando context === 'channels'.
    ...
    // 2. Sincronizza lo stato del pulsante (continua a fare da indicatore
    //    della modalità attiva per la lista canali). Le classi di collapse
    //    vengono rimosse: il long press che le imposta non è toccato da qui.
    ...
    // 3. Ri-renderizza la lista canali con la nuova modalità, SEMPRE.
    //    Anche se al momento è nascosta (siamo in playlist / EPG manager /
    //    Full EPG): così il DOM della lista canali è già coerente quando
    //    l'utente tornerà alla vista canali, senza mismatch tra classi e
    //    struttura e senza "scatti" al primo ritorno.
    //    Playlist, EPG manager e Full EPG NON vengono toccati: le loro viste
    //    sono già in list-view e non devono cambiare.
    renderGroupedChannelList(
        showingFavorites ? groupedFavoriteChannels : groupedChannels,
        { context: 'channels' }
    );

    // 4. Se la vista canali è attualmente visibile, riporta in vista il
    //    canale eventualmente attivo. Il check usa sia la classe `hidden`
    //    sia `style.display` per coprire tutti i casi (renderPlaylistList,
    //    renderEPGManager, switch ai tab inferiori).
    const channelListContainer = document.getElementById('channelListContainer');
...
// Precarica valori (fallback a stringa vuota: a questo punto dello script
// DEFAULT_PLAYLIST_URL/epgUrl sono ancora null, getConfigFromServer() non è
// stata ancora chiamata — verranno rivalorizzati correttamente più sotto,
// dentro DOMContentLoaded, appena la configurazione arriva dal server).
...
document.getElementById("toggleMetadata").onchange = e => {
  ...
  if (enabled) {
    // Attivazione ⇒ sempre espanso (stessa regola dell'EPG)
...
document.getElementById("toggleEPG").onchange = async e => {
  ...
  if (enabled) {
    // Attivazione ⇒ sempre espanso, a prescindere dallo stato precedente
...
// LISTENER: Reset settings

document.getElementById('resetListBtn').addEventListener('click', async () => {

  // ── 1. PROTEZIONE LICENZA ─────────────────────────────────
  // Tutto ciò che è legato alla licenza non va MAI toccato.
  const LICENSE_KEYS = ['zappone_license', 'zappone_jwt', 'zappone_device_id'];

  // ── 2. CANCELLA TUTTI I SETTINGS (tranne licenza) ─────────
  Object.keys(localStorage).forEach(key => {
    if (!LICENSE_KEYS.includes(key)) localStorage.removeItem(key);
  });

  // ── 3. REIMPOSTA I DEFAULT ESPLICITI ──────────────────────
  localStorage.setItem("zappone_show_metadata", "false");
  localStorage.setItem("metadataExpanded", "false");
  localStorage.setItem("zappone_show_epg", "false");
  localStorage.setItem("zappone_auto_fullscreen", "true");
  ...
  // ── 4. AGGIORNA TUTTI I TOGGLE DELLA SIDEBAR ──────────────
...
if (themeToggleSidebar) themeToggleSidebar.checked = true;
  ...
  // ── 5. AZZERA LO STATO IN MEMORIA ─────────────────────────
  // NB: favoriteChannels / groupedFavoriteChannels NON vengono toccati.
  showingFavorites = false;
  currentViewMode = 'list';
  groupCollapseState = {};
  favoriteGroupCollapseState = {};
  ...
  // ── 6. FORZA IL REFRESH DI TUTTE LE LISTE ─────────────────
  window.forcePlaylistReload = true;
  window.forceEPGReload = true;

  // ── 7. RICARICA LA PLAYLIST PREDEFINITA ───────────────────
  await loadRemoteM3U(DEFAULT_PLAYLIST_URL, true);

  // ── 8. AGGIORNA TUTTE LE UI ───────────────────────────────
  if (typeof refreshPlaylistUIs === 'function') refreshPlaylistUIs();
...
// passato, rispettiamo la sua scelta; altrimenti dark.
...
document.getElementById('fullEpgBtn').addEventListener('click', () => {
    ...
    // La visibilità dei container è gestita da renderFullEPGList stessa
    // (switchToView('fullEpg')), non serve più duplicarla qui.
    renderFullEPGList();
});
...
// Durata minima dello splash: l'utente vede comunque 2s di animazione,
// anche se l'app è pronta prima. Evita il classico "flash" dello splash.
const SPLASH_MIN_MS  = 2000;
const SPLASH_FADE_MS = 600; // deve essere >= durata transizione CSS (.55s)
...
// ================================
// FUNZIONE: Mostra il nome/stato della licenza nel tab Avanzate
// ================================
...
// 2b. Aggiorna la riga con il nome/stato della licenza nel tab Avanzate
    updateLicenseDisplay();

// 2c. Ora che la config è arrivata, DEFAULT_PLAYLIST_URL/epgUrl hanno un
// valore vero: ricarica i campi "URL predefinito" che a inizio script
// erano stati valorizzati troppo presto. Se l'utente ha già un suo
// default salvato in locale, quello resta comunque prioritario grazie
// all'||, quindi qui non si sovrascrive nulla che l'utente abbia scelto.
...
// 3. Inizializza il database locale, carica i preferiti e l'ultima vista
  ...
  // 3a. Togli lo splash: l'app è visibile e utilizzabile  
hideSplashScreen();   // ← fade-out appena l'app è pronta
