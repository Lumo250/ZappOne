...
// ============================================================
// LAZY LOADING — OBSERVER PER-CONTAINER
// Un IntersectionObserver dedicato per ogni container (channelList,
// playlistList, epgList, fullEpgList). Vengono memoizzati in una WeakMap:
// quando un container viene eliminato dal DOM, il suo observer viene GC'd.
// Ogni render ricrea gli item del proprio container senza toccare gli
// observer degli altri container.
// ============================================================
const _lazyObservers = new WeakMap();

function getObserverFor(container) {
  if (!container) return null;

  let obs = _lazyObservers.get(container);
  if (obs) return obs;
  ...
  obs = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      // <div data-bg-src="..."> (tile griglia Apple-TV)
      if (target.dataset && target.dataset.bgSrc) {
  ...
  _lazyObservers.set(container, obs);
  return obs;
}
...
// (getSharedObserver rimossa: sostituita da getObserverFor(container), vedi sopra)
...
function renderGroupedChannelList(groups, options = {}) {
    ...
    requestAnimationFrame(() => {
        ...
        // --- Observer per-container (memoizzato per targetElement) ---
        const lazyObserver = getObserverFor(targetElement);
...
// Ensure fallback background color while loading
        ...
        // Observe background image lazy loading (usa l'observer del container)
...
// 5. Feedback all'utente (usiamo appConfig per sicurezza)
    if (appConfig.isPremium) {
        showNotification("ZappOne Premium Attivo! V25.04");
    } else {
