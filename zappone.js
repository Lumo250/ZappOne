...
// imposta l'array VUOTO Sarà il server a riempire questi contenitori.
let isPremium = false;
...
// definisci un oggetto di stato globale
let appConfig = {
    status: 'demo',
    isPremium: false,
    playlistUrl: null,
...
async function getFingerprint() {
    ...
    localStorage.setItem('zappone_device_id', id);  // ← salva una volta sola
    return id;
}


// Funzione SEMPLICISSIMA Questa funzione ora è solo un "trasportatore" di dati.
async function getConfigFromServer() {
    let license = localStorage.getItem("zappone_license") || "demo_user";
    const deviceId = await getFingerprint(); // Ottieni l'ID unico
    ...
    try {
        // Inviamo sia license che deviceId
...
function rebuildEpgMap() {
    ...
    epgData.forEach(ch => {
        if (ch.name) {
            epgByName.set(normalizeChannelName(ch.name), ch);
        }
    });
}
...
// FUNZIONE BASE: scarica M3U da URL usando proxy multipli  (usa licenza)
async function fetchM3UWithProxies(url, returnType = 'text') {
    ...
    try {
        ...
        // Restituisci testo o ArrayBuffer in base al parametro
...
// FUNZIONE: Parsing della playlist M3U
...
const parsedData = await parseM3UInWorker(text);
        ...
        //  CORREZIONE: EPG in background COMUNQUE - assicurati che venga chiamato
...
function parseM3UInWorker(text) {
    ...
    let timeoutId;          // ← aggiungere questa riga    

        worker.postMessage({ text });
        
        worker.onmessage = function(e) {
 clearTimeout(timeoutId);  // ← aggiungere questa riga
            worker.terminate();
            if (e.data.success) {
                resolve(e.data);
            } else {
...
timeoutId = setTimeout(() => {   // ← cambiare setTimeout in timeoutId =
            worker.terminate();
...
const buffer = await fetchM3UWithProxies(targetUrl, 'arrayBuffer');
...
function showNotification(message, isError = false, force = false) {
  try {
    ...
    const maxLength = 150;
    notification.textContent = message.length > maxLength ? message.substring(0, maxLength) + '...' : message;
...
async function findChannelFromEPG(epgChannel) {
  ...
  // --- 3. SE ANCORA NON TROVATO, TENTATIVO MATCHING "SOFT" ---
  if (!foundChannel) {
...
// 5. Feedback all'utente (usiamo appConfig per sicurezza)
    if (appConfig.isPremium) {
        showNotification("ZappOne Premium Attivo! V23.00");
    } else {
