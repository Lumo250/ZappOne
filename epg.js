 ...
 // se non ci sono dati EPG
  ...
  epgDisplayList
    ...
    item.setAttribute('data-name', (epgChannel.name || '').toLowerCase());
       ...
       // Immagine della riga. La fonte EPG spesso ha i poster nei programmi
    // (<programme><icon>) ma NON i loghi nei canali (<channel><icon>): in quel
    // caso epgChannel.logo è vuoto e sotto al player funziona lo stesso perché
    // lì si usa program.poster. Applichiamo la stessa cascata di fallback qui.
    const currentProg = (epgChannel.programs || []).find(p => isProgramCurrentlyAiring(p));
    const imgUrl = epgChannel.logo
                || (currentProg && currentProg.poster)
                || 'img/placeholder.png';
    ...
    logo.onerror = function() {
      // removeAttribute invece di src='' — su Safari iOS quest'ultimo
      // innesca un secondo errore di caricamento e lascia il box grigio pieno
...
function openChannelEpgDrawer(epgChannel){
  ...
  // 1) Apri il drawer PRIMA di popolare: le <img> vengono create in un
  //    contenitore già visibile, così iOS Safari avvia il caricamento
  //    immediatamente (creare <img src> in un ramo display:none faceva
  //    fallire il load in modo silenzioso).
  drawer.classList.remove('hidden');
  drawer.classList.add('open');

  // 2) Ora resetta e popola
  ...
  // 3) Autoscroll al programma corrente
