...
let epgDisplayList = [];   // lista pronta per il Full EPG (con collisioni distinte rinominate)
let lastRenderedEpgData = null; // riferimento a epgData l'ultima volta che renderFullEPGList ha disegnato

// FIX B10 — Override EPG per il canale avviato dal ▶ del drawer EPG.
// Quando il ▶ trova in M3U un canale con nome diverso (es. via soft match)
// dalla voce EPG cliccata, il lookup per nome fallirebbe o troverebbe il
// primario sbagliato. L'override associa esplicitamente quel canale M3U
// alla voce EPG effettivamente visualizzata, per la durata della sessione
// di riproduzione. Viene azzerato all'uscita dal player (closeFullEPG),
// così se lo stesso canale viene poi lanciato dalla lista canali torna a
// valere la logica normale basata sul nome.
let _epgPlaybackOverride = null; // { channelKey, epgChannel }




// Confronta "cosa dovrebbe essere mostrato ora" con "cosa è stato disegnato
// l'ultima volta" (salvato come attributo sul container).
...
// FUNZIONE: Corregge URL immagine con più '?' nella query string.
...
function sanitizeImageUrl(url) {
  ...
  // 1. Fix dei separatori: '?checksum=...?tr=...' → '?checksum=...&tr=...'
  let fixed = url;
  ...
  // 2. Rewrite per i domini problematici su iOS: URL ASSOLUTO verso Netlify,
  // così funziona anche se l'app è servita da GitHub Pages o da un altro host.
  try {
            ...
            if (!activeEPG && successfulDownloads.length > 0) {
                ...
                if (firstSuccess) {
                    ...
                    if (epgRec) {
                        ...
                        if (epgRec.content) {
                            try {
                                const data = JSON.parse(epgRec.content);
                                epgData = data;
...
// Confronta due voci EPG per determinare se i loro palinsesti sono identici.
// Usato da rebuildEpgMap per distinguere:
//   - canali "duplicati" (stesso palinsesto) → la voce in eccesso viene scartata
//   - canali realmente distinti (palinsesto diverso) → la voce in eccesso viene
//     rinominata con un suffisso " (+N)" e mostrata come canale separato
// Il confronto è per (start, end, title) nell'ordine in cui appaiono.
...
function rebuildEpgMap() {
    ...
    // Per ogni chiave normalizzata teniamo un contatore delle voci
    // "distinte" già incontrate. La prima occorrenza diventa il primario
    // (quello che finisce in epgByName e risponde ai lookup per nome della
    // playlist). Le successive:
    //   - se hanno programmi identici al primario → scartate (comportamento
    //     preesistente: si tiene quella con più programmi);
    //   - se hanno programmi diversi → tenute come canali distinti, con
    //     nome suffissato " (+N)" in epgDisplayList.
    ...
    epgData.forEach(ch => {
        ...
        if (!existing) {
            // Prima occorrenza per questa chiave: è il primario.
        ...
        if (programsAreIdentical(existing, ch)) {
            // Stesso palinsesto: si tiene quella con più programmi (comportamento
            // preesistente). L'altra non finisce in epgDisplayList.
        ...
        // Palinsesto diverso: canale realmente distinto, si aggiunge con suffisso.
...
const activeEPG = await getActiveEPG();
// FIX: includi lo stato del toggle "Mostra URL" nel fingerprint.
// Senza questo, cambiare il toggle non modifica il fingerprint (basato solo
// su id/lastFetched) e la funzione esce senza ridisegnare la lista, lasciando
// visibili gli URL anche dopo la disattivazione.
...
const fingerprint = `url:${showUrlMode ? 1 : 0}|${(epgs || []).map(e => `${e.id}:${e.lastFetched || 0}`).join('|')}`;
    ...
    // 3. GRUPPI
    const groups = [
        { name: "Caricamento", channels: [
            { name: "Aggiungi EPG", logo: "add-epg.svg", url: "#url", __special: true }
        ]},
    ...
    renderSavedItemsList(epgList, groups, {
            ...
            return [
                {
                    ...
                    onClick: async (itemData, itemEl, iconEl) => {
                        try {
                            ...
                            showNotification(`EPG "${itemData.name}" aggiornato`);
        ...
        onItemClick: async (def, itemEl) => {
            ...
            // Se è già quello attivo, passa solo alla vista Full EPG
            ...
            // Evidenzia SUBITO l'elemento cliccato, prima ancora di scaricare
            // o attivare il contenuto: così, tornando a questa lista, la si
            // ritrova già corretta invece di vedere lo scatto dell'evidenziazione.
            if (itemEl) {
...
function renderFullEPGList() {
  ...
  // epgData viene sempre riassegnato per intero (mai modificato sul posto):
  // il confronto di riferimento basta a sapere se c'è qualcosa di nuovo.
 ...
 // se non ci sono dati EPG
  ...
  // La vista Full EPG è SEMPRE in modalità lista: la preferenza list/grid
  // dell'utente vale solo per la lista canali.
  ...
  epgDisplayList
    ...
    // Sanifica l'URL: gli EPG Sky contengono URL malformati (es. doppio '?')
    // che Safari iOS rifiuta a livello di parsing. Vedi sanitizeImageUrl.
    logo.dataset.src = sanitizeImageUrl(epgChannel.logo) || 'img/placeholder.png';
    ...
    // NB: niente getCurrentProgramInfo qui, perché per una voce rinominata
    // ("Rai 1 HD (+1)") la normalizzazione tornerebbe alla chiave del primario
    // ("rai1") e mostrerebbe il programma del primario. Usiamo invece i
    // programmi della voce stessa.
...
function showChannelEPG(channel) {
  ...
  // FIX B10: se questo canale è stato avviato dal ▶ di una voce EPG, usa
// quella voce. Altrimenti fallback al lookup per nome (comportamento normale).
let channelEPG;
if (_epgPlaybackOverride && _epgPlaybackOverride.channelKey === getChannelKey(channel)) {
    channelEPG = _epgPlaybackOverride.epgChannel;
} else {
    channelEPG = epgByName.get(normalizeChannelName(channel.name));
}
  ...
  nextPrograms.forEach(program => {
    ...
    // Sanifica gli URL (vedi sanitizeImageUrl): gli EPG Sky contengono URL
    // con doppio '?' che Safari iOS rifiuta a livello di parsing.
    img.src = sanitizeImageUrl(program.poster) || sanitizeImageUrl(channel.logo) || 'placeholder.png';
    img.alt = '';
...
function openChannelEpgDrawer(epgChannel){
  ...
  epgChannel.programs.forEach(pr => {
    ...
    // Sanifica gli URL (vedi sanitizeImageUrl): gli EPG Sky contengono URL
    // con doppio '?' che Safari iOS rifiuta a livello di parsing.
    img.src = sanitizeImageUrl(pr.poster) || sanitizeImageUrl(epgChannel.logo) || 'img/placeholder.png';
    img.alt = '';
...
playBtn.addEventListener('click', async (e) => {
    ...
    //    Niente loadEPG: premere Play non deve far scaricare EPG.
    if (!channels || channels.length === 0) {
        ...
        await loadFavorites();
        // parseM3U ha già chiamato rebuildIndexMaps: non ripetere.
    }
...
if (found && found.channel) {
    ...
    // FIX B10: registra l'associazione esplicita tra il canale M3U che
    // stiamo per avviare e la voce EPG da cui è partito il Play. showChannelEPG
    // userà questa associazione al posto del lookup per nome.
    // NB: va DOPO closeFullEPG(), che azzera l'override.
...
function closeFullEPG() {
    // FIX B10: uscendo dal player/Full EPG termina la sessione di riproduzione.
    // L'override del ▶ non deve sopravvivere a un successivo lancio dalla lista.
