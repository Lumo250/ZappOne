...
async function loadRemoteM3U(url, closeAfter = false) {
 //  Chiudi il bottom sheet se richiesto
    ...
    // PROTEZIONE LICENZA: in demo è consentito solo l'URL di playlist
    // fornito dal server (appConfig.playlistUrl); qualunque altro URL
    // (es. salvato manualmente come "predefinito" nelle Impostazioni
    // Avanzate) richiede Premium. cors-proxy.js applica comunque lo stesso
    // limite: questo blocco serve solo a dare un messaggio chiaro invece
    // di un generico errore di rete.
    if (!appConfig.isPremium && url !== appConfig.playlistUrl) {
        alert("Il caricamento di playlist da URL personalizzati è riservato agli utenti Premium.");
        return;
    }
...
document.getElementById("modalConfirm").addEventListener("click", async function() {
    ...
    // PROTEZIONE LICENZA: questo flusso serve solo ad aggiungere una NUOVA
    // playlist da un URL scelto dall'utente, quindi qui non c'è un
    // "default" da distinguere: in demo è sempre bloccato.
    if (!appConfig.isPremium) {
        alert("Il caricamento di playlist da URL personalizzati è riservato agli utenti Premium.");
        return;
    }
