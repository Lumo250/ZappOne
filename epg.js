...
// ================================
// Placeholder SVG inline per loghi/poster mancanti.
// Usato in TUTTI i punti che prima puntavano a 'img/placeholder.png' o
// 'tasto-icon.png', file che non esistono tra gli asset statici e
// causavano quindi una richiesta HTTP 404 per ogni logo rotto.
// Essendo un data-URI, non genera traffico di rete ed è sempre disponibile
// anche offline (PWA).
const PLACEHOLDER_LOGO_SVG = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="%23888" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="14" rx="2"/><path d="M8 22h8M12 18v4"/></svg>';

// FUNZIONE: Normalizza un URL di logo/poster EPG.
// Molte fonti XMLTV/JSON usano URL relativi ("logos/rai1.png") oppure
// protocol-relative ("//cdn.example/x.png"), oppure ancora http:// mentre
// la pagina è servita in https:// (mixed content, bloccato dal browser).
// Questa funzione risolve l'URL rispetto all'EPG di origine e promuove
// http->https quando la pagina è https, in modo che le immagini siano
// effettivamente caricabili.
function resolveEpgUrl(rawUrl, baseUrl) {
  ...
  if (trimmed.startsWith('//')) {
    let proto = 'https:';
    if (baseUrl) {
      try { proto = new URL(baseUrl).protocol; } catch (e) { /* keep default */ }
    }
  ...
  if (/^https?:\/\//i.test(trimmed)) {
    // Upgrade http -> https quando la pagina è https, per evitare
    // il blocco mixed-content del browser.
    if (trimmed.startsWith('http://') && baseUrl && baseUrl.startsWith('https://')) {
      return 'https://' + trimmed.slice(7);
    }
    return trimmed;
  }

  // Relativo: risolvi rispetto all'URL dell'EPG sorgente
  if (baseUrl) {
    try { return new URL(trimmed, baseUrl).href; } catch (e) { /* fallthrough */ }
  }
  return trimmed;
}

// FUNZIONE: Applica resolveEpgUrl a tutti i loghi/poster di una struttura
// JSON EPG (array di canali con programs).
function resolveEpgDataUrls(data, baseUrl) {
  if (!Array.isArray(data)) return data;
  return data.map(ch => {
    if (!ch || typeof ch !== 'object') return ch;
    return {
      ...ch,
      logo: resolveEpgUrl(ch.logo, baseUrl),
      programs: Array.isArray(ch.programs)
        ? ch.programs.map(p => (p && typeof p === 'object')
            ? { ...p, poster: resolveEpgUrl(p.poster, baseUrl) }
            : p)
        : ch.programs
    };
  });
  ...
  try {
    ...
    try {
      if (isXML) {
        ...
        // FIX: passo l'URL sorgente a parseXMLTV per risolvere eventuali URL
        // relativi di loghi canale e poster programma (es. "logos/x.png")
        // rispetto alla posizione reale dell'EPG.
        data = parseXMLTV(xmlDoc, targetUrl);
      } else {
        // FIX: applico la stessa risoluzione URL anche al ramo JSON.
        data = resolveEpgDataUrls(JSON.parse(responseText), targetUrl);
      }
    } catch (parseError) {
...
// FIX: aggiunto parametro opzionale `baseUrl` per risolvere correttamente
// i loghi (relativi) dei canali e i poster (relativi) dei programmi.
function parseXMLTV(xmlDoc, baseUrl = '') {
  const channels = Array.from(xmlDoc.getElementsByTagName('channel')).map(channel => {
    return {
      ...
      logo: resolveEpgUrl(channel.getElementsByTagName('icon')[0]?.getAttribute('src'), baseUrl)
    };
  });
  ...
  const programmes = Array.from(xmlDoc.getElementsByTagName('programme')).map(programme => {
    ...
    return {
      ...
      poster: resolveEpgUrl(programme.getElementsByTagName('icon')[0]?.getAttribute('src'), baseUrl)
    };
  })
...
function renderFullEPGList() {
 ...
 // se non ci sono dati EPG
  ...
  // Observer per-container (memoizzato per #fullEpgList)
  const lazyObserver = getObserverFor(list);
  ...
  epgData.forEach(epgChannel => {
    ...
    // ── LOGO CANALE ──
    // FIX 1: se l'EPG non fornisce un logo per il canale, uso come fallback
    // il logo del canale omonimo nella playlist (già presente in memoria),
    // e solo in ultima istanza il placeholder SVG inline.
    // In questo modo, per le fonti EPG che emettono solo i programmi (senza
    // <channel><icon>), la vista Full EPG mostra comunque un'immagine utile.
    let logoUrl = epgChannel.logo;
    if (!logoUrl && typeof channels !== 'undefined' && Array.isArray(channels)) {
      const normName = normalizeChannelName(epgChannel.name);
      const playlistChannel = channels.find(c => normalizeChannelName(c.name) === normName);
      if (playlistChannel && playlistChannel.logo) {
        logoUrl = playlistChannel.logo;
      }
    }
    if (!logoUrl) logoUrl = PLACEHOLDER_LOGO_SVG;
    ...
    logo.onerror = function() {
      this.onerror = null;               // evita re-trigger
      this.removeAttribute('src');       // niente load implicito del documento
...
nextPrograms.forEach(program => {
  ...
  // FIX 2: fallback al placeholder SVG inline invece di 'placeholder.png'
  // (file inesistente che causava 404 e icona "immagine rotta").
  img.src = program.poster || channel.logo || PLACEHOLDER_LOGO_SVG;
  img.alt = '';
  // FIX 3: onerror per sostituire un poster rotto con il placeholder,
  // evitando l'icona rotta del browser.
  img.onerror = function() {
    this.onerror = null;
    this.src = PLACEHOLDER_LOGO_SVG;
  };
...
const img = document.createElement('img');
img.className = 'epg-poster';
// FIX 2: fallback al placeholder SVG inline invece di 'img/placeholder.png'
img.src = pr.poster || epgChannel.logo || PLACEHOLDER_LOGO_SVG;
img.alt = '';
// FIX 3: onerror per evitare l'icona "immagine rotta" quando il poster 404.
// Il drawer era l'unico punto senza handler di errore, quindi ogni poster
// mancante produceva il simbolo di file rotto invece di un box pulito.
img.onerror = function() {
  this.onerror = null;
  this.src = PLACEHOLDER_LOGO_SVG;
};
