...
// ============================================================
// EQUALIZZATORE AUDIO (Web Audio API)
// ============================================================
// Un solo AudioContext condiviso, con una catena di 3 filtri per l'elemento
// video e una per l'elemento audio (radio). createMediaElementSource() si
// può chiamare una sola volta per elemento — qui lo facciamo la prima volta
// che quell'elemento viene usato, poi la catena resta agganciata per tutta
// la vita dell'app: cambiare canale non richiede di rifare nulla.
//
// Tutto è avvolto in try/catch: se la creazione fallisce per qualunque
// motivo, l'elemento continua a suonare per la via normale, senza passare
// dalla Web Audio API — non deve MAI risultare in silenzio per colpa
// dell'equalizzatore.
...
const EQ_PRESETS = {
  ...
  bassi:   { bass: 7,  mid: 0,  treble: -1 },
  parlato: { bass: -4, mid: 5,  treble: -2 },
  film:    { bass: 4,  mid: -2, treble: 3  }
};
...
function ensureEqChain(mediaElement, state) {
  ...
  try {
    ...
    // Applica subito il preset salvato (o "normale" se non c'è ancora nulla)
