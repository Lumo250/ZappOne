...
async function fetchEPGBinary(targetUrl) {
  // Il fetch diretto salta la licenza (nessuna richiesta passa da
  // cors-proxy.js): va tentato SOLO se l'utente è Premium oppure se l'URL
  // è quello di default del server. Per un URL personalizzato in demo si
  // passa sempre dal proxy, che applica la verifica della licenza esattamente
  // come per le playlist M3U (vedi Patch 1 di cors-proxy.js).
  const canSkipProxy = appConfig.isPremium || targetUrl === appConfig.epgUrl;
  ...
  if (canSkipProxy) {
    ...
    console.log('URL EPG personalizzato in demo: uso il proxy (verifica licenza).');
  }
...
async function downloadEPG(url = appConfig.epgUrl, options = {}) {
  ...
  // PROTEZIONE LICENZA: in demo è consentito solo l'URL EPG fornito dal
  // server (appConfig.epgUrl); qualunque altro URL richiede Premium.
  // Vedi anche fetchEPGBinary(), che per lo stesso motivo evita il fetch
  // diretto (bypass del proxy/licenza) per gli URL non di default in demo.
  if (!appConfig.isPremium && targetUrl !== appConfig.epgUrl) {
    if (!silent) showNotification('Il caricamento di EPG da URL personalizzati è riservato agli utenti Premium.', true);
    return null;
  }
...
function renderFullEPGList() {
  ...
  // Scollega SOLO le immagini di questa lista dall'observer condiviso, senza
  // toccare quelle di altre liste (channel list, playlist list, ecc.) che
  // condividono lo stesso observer. Disconnettere l'observer condiviso qui
  // lasciava orfane le immagini in lazy-loading delle altre liste, che non
  // ricevevano più il loro src = data-src.
