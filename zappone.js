...
// ============================================================
// NUOVA VERSIONE DI processSwipe CON NAVIGAZIONE TRA VISTE
// ============================================================
// ============================================================
// processSwipe con navigazione tra viste (versione definitiva)
// ============================================================
function processSwipe(dx, dy) {
  ...
  const isPlaylistView = window.isPlaylistView;
  const isEPGView = window.isEPGView;
  const isFullEPG = !document.getElementById('fullEpgContainer').classList.contains('hidden');

  // --- PLAYLIST ↔ CANALI ---
  if (isPlaylistView) {
    // Swipe left (dx < 0) -> go to channels
    if (dx < 0) {
  ...
  // --- EPG MANAGER → FULL EPG ---
  if (isEPGView) {
    // Swipe left (dx < 0) -> go to full EPG
    if (dx < 0) {
      // Nascondi l'EPG Manager e mostra il Full EPG
      ...
      if (epgListContainer) epgListContainer.classList.add('hidden');
      if (fullEpgContainer) {
        fullEpgContainer.classList.remove('hidden');
        if (typeof renderFullEPGList === 'function') renderFullEPGList();
      }
      // Aggiorna i flag di vista
      window.isEPGView = false;  // Non siamo più nell'EPG manager, ma nel Full EPG
      window.isPlaylistView = false;
    }
    return;
  }

  // --- FULL EPG → EPG MANAGER ---
  if (isFullEPG) {
    // Swipe right (dx > 0) -> go back to EPG manager
    if (dx > 0) {
      if (typeof renderEPGManager === 'function') {
        // renderEPGManager() gestisce già la visibilità dei container e i flag
        renderEPGManager();
      }
    }
    return;
  ...
  // --- We are in CHANNELS view (default) ---
  // From channels, swipe right (dx > 0) -> go to playlist list
  if (dx > 0) {
    if (typeof renderPlaylistList === 'function') renderPlaylistList();
    return;
  }

  // If swipe left in channels, toggle between list/grid (existing behavior)
  if (dx < 0 && currentViewMode === 'list') {
    switchViewMode('grid');
  } else if (dx > 0 && currentViewMode === 'grid') {
    // già gestito sopra (vai alle playlist)
  }
}
...
if (savedEPG === "true") {
  ...
  document.getElementById("toggleEPG").checked = false;
